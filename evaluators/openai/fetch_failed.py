...
if __name__ == "__main__":
    ...
    assert len(all_batches) == 140

    all_batches = all_batches[:138]

    # Fetch failed batches
    os.makedirs("batch_tasks_output/openai/failed", exist_ok=True)
    ...
    for batch in tqdm(all_batches):
        ...
        content = model.client.files.content(file_id)
        time.sleep(0.5)
        task = batch.metadata["task"]
        file_id = batch.metadata["file_id"]
        out_basepath = f"batch_tasks_output/openai/failed"
